...
struct RootView: View {
    ...
    var body: some View {
        NavigationSplitView {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Spacer()
                Text("Un pomodoro alla volta.").font(.caption).foregroundStyle(.secondary).padding(.horizontal, 12)
    ...
    private func icon(_ tab: AppTab) -> String {
        switch tab { case .timer: return "timer"; case .calendar: return "calendar"; case .history: return "chart.bar.xaxis"; case .settings: return "slider.horizontal.3" }
    }
}
        ...
        VStack(alignment: .leading, spacing: 20) {
            ...
            HStack(spacing: 28) {
                ...
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    TextField("Scrivi un'attività", text: $title).textFieldStyle(.plain).padding(14).background(.white.opacity(0.7), in: RoundedRectangle(cornerRadius: 14))
                    Text("Sessione \(engine.sessionNumber) di \(settings.dailyGoal)").foregroundStyle(.secondary)
                    Button(engine.running ? "Pausa" : "Inizia sessione") { engine.running ? engine.pause() : engine.start() }.buttonStyle(.borderedProminent).tint(FoodapomoStyle.orange).controlSize(.large)
                    Button("Termina sessione") { engine.stop() }.buttonStyle(.bordered).disabled(!engine.running)
                }.frame(maxWidth: 280, alignment: .leading)
            }.frame(maxWidth: .infinity, maxHeight: .infinity)
            HStack {
                Label("Prossima pausa", systemImage: "cup.and.saucer.fill").foregroundStyle(.secondary)
                Spacer()
                Text("Pausa breve tra \(max(1, settings.sessionsBeforeLongBreak - (engine.sessionNumber - 1) % max(1, settings.sessionsBeforeLongBreak))) sessioni").font(.subheadline.bold())
            }.padding(18).background(FoodapomoStyle.green.opacity(0.65), in: RoundedRectangle(cornerRadius: 18))
        }.padding(34).onAppear { setup() }
    }

    private var timerCard: some View {
        ZStack {
            Circle().stroke(Color.black.opacity(0.06), lineWidth: 22)
            Circle().trim(from: 0, to: engine.progress).stroke(FoodapomoStyle.orange, style: StrokeStyle(lineWidth: 22, lineCap: .round)).rotationEffect(.degrees(-90)).animation(.easeInOut, value: engine.progress)
            VStack(spacing: 8) {
                TomatoMark(size: 48)
                Text(engine.formatted).font(.system(size: 62, weight: .medium, design: .rounded))
                Text(engine.isBreak ? "Pausa" : (title.isEmpty ? "Pronto?" : title)).font(.title3.weight(.medium)).multilineTextAlignment(.center).lineLimit(2)
            }
        }.frame(width: 390, height: 390).padding(22).background(.white.opacity(0.42), in: RoundedRectangle(cornerRadius: 30))
    }
...
struct MenuBarView: View {
    ...
    var body: some View {
        VStack(spacing: 14) {
            ...
            Button(engine.running ? "Pausa" : "Avvia") { engine.running ? engine.pause() : engine.start() }.buttonStyle(.borderedProminent).tint(FoodapomoStyle.orange)
        }.padding(20).frame(width: 220).background(FoodapomoStyle.cream)
    }
}
